...
|---|---|---|
| `panel.schedules` | `ScheduleAPI` | Create, update, delete, and run scheduled tasks. See [ScheduleAPI](/api-reference/api-schedules). |
| `panel.files` | `FileManagerAPI` | List, upload, download, copy, move, compress, and delete server files. See [FileManagerAPI](/api-reference/api-files). |
| `panel.backups` | `BackupsAPI` | List, create, download, and delete server backups. See [BackupsAPI](/api-reference/api-backups). |
| `panel.databases` | `DatabasesAPI` | List, create, rotate passwords for, and delete server databases. See [DatabasesAPI](/api-reference/api-databases). |
| `panel.startup` | `StartupAPI` | Read and update server startup variables. See [StartupAPI](/api-reference/api-startup). |
| `panel.commands` | `CommandAPI` | Send raw console commands to a server. See [CommandAPI](/api-reference/api-commands). |
