...
`MessageRouter` is a structured logging and message-routing utility that fans a single log call out to multiple destinations simultaneously: the local terminal (`origin`), every server's HungerBridge console (`destination`), every server's in-game chat (`broadcast`), and a rotating date-stamped log file (`filelog`). Color and format transformations are applied per-destination using `mapres` map lists, so ANSI escape codes appear in the terminal, Minecraft color codes appear in chat, and plain stripped text is written to disk.
