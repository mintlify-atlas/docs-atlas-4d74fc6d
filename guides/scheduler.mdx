---
title: "Schedule Events and Wait for Server States"
sidebarTitle: "Scheduler"
description: "HungerLib's scheduling utilities snap events to clock boundaries, run countdown callbacks at precise intervals, and block until servers reach a target state."
---
...
`snapSchedule` looks at the current time, adds the required lead time, then finds the earliest minute value from `snapMinutes` that the result still falls before. If no snap minute within the current hour qualifies, it rolls over to the top of the next hour. This guarantees scheduled events always land on a clean, predictable boundary rather than an arbitrary offset.
