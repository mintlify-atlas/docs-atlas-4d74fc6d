...
**Returns** `float` — current RAM usage in MB (or GB if `gb=True`), or `None` if the metric is unavailable.
...
**Returns** `float` — current CPU usage as an absolute percentage (e.g. `45.67`), or `None` if unavailable.
...
**Returns** `float` — current disk usage in MB or GB, or `None` if unavailable.
...
**Returns** `float` — cumulative inbound network usage in MB or GB, or `None` if unavailable.
...
**Returns** `float` — cumulative outbound network usage in MB or GB, or `None` if unavailable.
...
```python
server.powerAction(action: str) -> tuple[int, str]
```
...
**Returns** `tuple[int, str]` — `(status_code, response_body)`.
...
```python
server.createSchedule(payload: dict) -> requests.Response
```
...
```python
server.updateSchedule(schedule_id: str, payload: dict) -> requests.Response
```
...
<ParamField path="payload" type="dict" required>
  Dict of fields to update on the schedule.
</ParamField>
